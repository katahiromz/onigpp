...
void TestNonMemberSwap() {
	...
	rex::regex re1("pattern1 (\\d+)");
	rex::regex re2("pattern2 (\\w+)");

	// Test re1 before swap
	std::string subject1 = "pattern1 123";
	smatch m1;
	bool found1 = rex::regex_search(subject1, m1, re1);
	assert(found1);
	assert(m1.size() == 2);
	...
	smatch m2;
	bool found2 = rex::regex_search(subject2, m2, re2);
	assert(found2);
	assert(m2.size() == 2);
	...
	smatch m3;
	bool found3 = rex::regex_search(subject2, m3, re1);
	assert(found3);
	assert(m3.size() == 2);
	...
	smatch m4;
	bool found4 = rex::regex_search(subject1, m4, re2);
	assert(found4);
	assert(m4.size() == 2);
...
void TestSwapWithStdSwap() {
	...
	assert(rex::regex_match(subject1.begin(), subject1.end(), re1));

	// Test re2 before swap
	const std::string subject2 = "beta";
	assert(rex::regex_match(subject2.begin(), subject2.end(), re2));
	...
	assert(!rex::regex_match(subject1.begin(), subject1.end(), re1));
	assert(rex::regex_match(subject2.begin(), subject2.end(), re1));

	// Test re2 after swap (should now match "alpha")
	assert(rex::regex_match(subject1.begin(), subject1.end(), re2));
	assert(!rex::regex_match(subject2.begin(), subject2.end(), re2));
...
void TestSwapWithWideCharacters() {
	...
	rex::wregex wre1(L"test1");
	rex::wregex wre2(L"test2");

	// Test wre1 before swap
	const std::wstring wsubject1 = L"test1";
	assert(rex::regex_match(wsubject1.begin(), wsubject1.end(), wre1));

	// Test wre2 before swap
	const std::wstring wsubject2 = L"test2";
	assert(rex::regex_match(wsubject2.begin(), wsubject2.end(), wre2));
	...
	assert(!rex::regex_match(wsubject1.begin(), wsubject1.end(), wre1));
	assert(rex::regex_match(wsubject2.begin(), wsubject2.end(), wre1));

	// Test wre2 after swap (should now match "test1")
	assert(rex::regex_match(wsubject1.begin(), wsubject1.end(), wre2));
	assert(!rex::regex_match(wsubject2.begin(), wsubject2.end(), wre2));
...
void TestSwapWithFlags() {
	...
	rex::regex re1(pattern1, rex::regex_constants::icase);
#ifdef USE_STD_FOR_TESTS
	// std::regex_constants doesn't have multiline flag, use ECMAScript (default) instead
	rex::regex re2(pattern2, rex::regex_constants::ECMAScript);
#else
	rex::regex re2(pattern2, rex::regex_constants::multiline);
#endif

	// Verify flags before swap
#ifndef USE_STD_FOR_TESTS
	assert(re1.flags() == rex::regex_constants::icase);
	assert(re2.flags() == rex::regex_constants::multiline);
#endif

	// Test matching behavior before swap
	const std::string subject1 = "TEST";
	assert(rex::regex_match(subject1.begin(), subject1.end(), re1)); // icase allows matching
...
#ifndef USE_STD_FOR_TESTS
	assert(re1.flags() == rex::regex_constants::multiline);
	assert(re2.flags() == rex::regex_constants::icase);
#endif

	// Test matching behavior after swap
	assert(rex::regex_match(subject1.begin(), subject1.end(), re2)); // re2 now has icase flag
