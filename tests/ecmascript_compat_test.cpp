...
void TestDotBehaviorDefault() {
	...
	sregex re(pattern1, rex::regex_constants::ECMAScript);
	smatch m;
	...
	sregex re2(pattern2, rex::regex_constants::ECMAScript);
	smatch m2;
	bool found2 = rex::regex_search(text, m2, re2);
...
void TestMultilineAnchors() {
	...
	sregex re1(pattern1, rex::regex_constants::ECMAScript);
	smatch m1;
	assert(rex::regex_search(text, m1, re1)); // Should match at start
	...
	sregex re2(pattern2, rex::regex_constants::ECMAScript);
	smatch m2;
	assert(rex::regex_search(text, m2, re2)); // Should match at end
	...
	sregex re3(pattern3, rex::regex_constants::ECMAScript);
#else
	sregex re3(pattern3, rex::regex_constants::ECMAScript | rex::regex_constants::multiline);
#endif
	smatch m3;
	assert(!rex::regex_search(text, m3, re3)); // Should not match because dot doesn't match \n
...
void TestHexEscapes() {
	...
	assert(rex::regex_search(text1, m1, re1));
	assert(m1[0].str() == "ABC");
	...
	assert(rex::regex_search(text3, m3, re3));
...
void TestNoPreprocessingWithoutECMAScript() {
	...
	try {
		...
		sregex re(pattern, rex::regex_constants::basic);
		smatch m;
...
void TestCombinedFeatures() {
	...
	std::string pattern1 = "\\x41bc";
	sregex re1(pattern1, rex::regex_constants::ECMAScript | rex::regex_constants::icase);
	smatch m1;
	std::string text1 = "ABC";
	assert(rex::regex_search(text1, m1, re1));
	assert(m1[0].str() == "ABC");
...
void TestReplacementTemplate() {
	...
	sregex re1(pattern1, rex::regex_constants::ECMAScript);
	std::string fmt1 = "$2, $1";
	std::string result1 = rex::regex_replace(text1, re1, fmt1);
	...
	sregex re2(pattern2, rex::regex_constants::ECMAScript);
	std::string fmt2 = "[$&]";
	std::string result2 = rex::regex_replace(text2, re2, fmt2);
	...
	sregex re3(pattern3, rex::regex_constants::ECMAScript);
	std::string fmt3 = "($`)[$&]($')";
	std::string result3 = rex::regex_replace(text3, re3, fmt3);
	// Result breakdown:
	...
	sregex re4(pattern4, rex::regex_constants::ECMAScript);
	std::string fmt4 = "$$$&";  // First $$ = literal "$", then $& = whole match "100"
	std::string result4 = rex::regex_replace(text4, re4, fmt4);
...
void TestQuantifiers() {
	...
	assert(rex::regex_search(text3a, m3, re3));
	assert(m3[0].str() == "color");
	assert(rex::regex_search(text3b, m3, re3));
	...
	assert(rex::regex_search(text4, m4, re4));
...
void TestLookaheadAssertions() {
	...
	smatch m2b;
	assert(rex::regex_search(text2b, m2b, re2));
	assert(m2b[0].str() == "50");
